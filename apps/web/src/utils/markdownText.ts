...
export function formatReadTimeFromWordCount(wordCount: number, wordsPerMinute: number = 200): string {
  if (wordCount <= 0) {
    return ''
  }
...
function getMatchExcerpt(content: string, matchIndex: number, queryLength: number, radius: number = 72): string {
  const start = Math.max(0, matchIndex - radius)
  const end = Math.min(content.length, matchIndex + queryLength + radius)
  return content.slice(start, end).replace(/\s+/g, ' ').trim()
}

export function getMarkdownExcerpt(content: string = '', query: string = '', fallback: string = 'No content yet.'): string {
