...
type TimelineGroup = { label: string; notes: NoteFile[] };

function groupNotesByTimeline(notes: NoteFile[]): TimelineGroup[] {
	const now = new Date();
	const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
	const today: NoteFile[] = [];
	const yesterday: NoteFile[] = [];
	const thisWeek: NoteFile[] = [];
	const thisMonth: NoteFile[] = [];
	const older: NoteFile[] = [];

	for (const note of notes) {
		const date = new Date(note.updatedAt || note.createdAt || '');
		const startOfNoteDay = new Date(date.getFullYear(), date.getMonth(), date.getDate());
		const dayDiff = Math.floor((startOfToday.getTime() - startOfNoteDay.getTime()) / 86400000);
		if (dayDiff === 0) today.push(note);
		else if (dayDiff === 1) yesterday.push(note);
		else if (dayDiff < 7) thisWeek.push(note);
		else if (date.getMonth() === now.getMonth() && date.getFullYear() === now.getFullYear()) thisMonth.push(note);
		else older.push(note);
	}

	const groups: TimelineGroup[] = [];
	if (today.length) groups.push({ label: 'Today', notes: today });
	if (yesterday.length) groups.push({ label: 'Yesterday', notes: yesterday });
	if (thisWeek.length) groups.push({ label: 'This Week', notes: thisWeek });
	if (thisMonth.length) groups.push({ label: 'This Month', notes: thisMonth });
	if (older.length) groups.push({ label: 'Older', notes: older });
	return groups;
}
...
export default function NoteEditor({
	...
	if (!note) {
		const recentNotes = [...fileNotes].sort(compareRecentNotes);
		const groupedRecentNotes = groupNotesByTimeline(recentNotes);
						...
						<div className="relative overflow-hidden">
							<AnimatePresence mode="wait" initial={false}>
								{homeTab === 'recent' ? (
									...
									>
										{groupedRecentNotes.length > 0 ? (
											<div className="flex flex-col gap-4">
												{groupedRecentNotes.map((group) => {
													const isOlder = group.label === 'Older';
													const visibleNotes = isOlder && !olderExpanded ? group.notes.slice(0, 2) : group.notes;
													return (
														<div key={group.label}>
															...
															{isOlder && group.notes.length > 2 && (
																<button
																	type="button"
																	onClick={() => setOlderExpanded((v) => !v)}
																	className="mt-1 w-full rounded-xl py-2 text-[12px] font-medium text-[var(--text-muted)] transition-colors duration-150 hover:text-[var(--text-primary)] hover:bg-[var(--bg-hover)] active:scale-[0.96]"
																	style={{ WebkitTapHighlightColor: 'transparent' }}
																>
																	{olderExpanded ? 'Show less' : `Show more (${group.notes.length - 2})`}
																</button>
															)}
														</div>
													);
